...
def map_lines(original_lines, new_lines):
    ...
    for i, old_line in enumerate(original_lines):
        ...
        # Optionally skip totally-empty normalized lines to avoid noise.
        # Comment out this block if you ALSO want blank/comment-only lines mapped.
        if old_line == "":
            continue

        new_num = -1  # default: not found / changed

        for j, new_line in enumerate(new_lines):
            if old_line == new_line:
                new_num = j + 1
                # First match only (same behavior as original C logic)
                break

        mappings.append((old_num, new_num))
